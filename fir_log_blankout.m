function [W,theta] = fir_log_blankout(XQ,XP,yQ,lambda)
...
if K==2
    ...
    w = minFunc(@fir_log_blankout_grad, zeros(length(ix),1), options, XQ(ix,:),yQ,theta(ix),lambda);
    ...
    for k = 1:K
        ...
        w = minFunc(@fir_log_blankout_grad, zeros(length(ix),1), options, Xk(ix,:),yk,theta{k}(ix),lambda);
